...
<Meta title="Drag And Resize/Resize" />
...
The events you get from `<Day>` have some extra props that you can use to customize the dragging and resizing behavior, and the styling.
...
<Source
  code={`
const Event = (
  { id, title, top, bottom, className, start, end, startsBeforeToday, endsAfterToday }
) => {
