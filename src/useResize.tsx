...
import { addMinutes, differenceInMinutes, isSameDay, roundToNearestMinutes } from 'date-fns'
...
export function useResize(eventId: string, roundMinutes = 15) {
  ...
  const handleDrag = useCallback((e: DragEvent<HTMLElement>) => {
    ...
    const mouseDate = pixelsToDate(e.clientY - topRef.current, columnHeight, date)
    const finalNewEndDate = roundToNearestMinutes(addMinutes(mouseDate, offsetFromEnd.current), { nearestTo: roundMinutes })

    if (!isSameDay(event.end, finalNewEndDate)) return
    if (differenceInMinutes(finalNewEndDate, event.start) < 120) return

    onEventChange({
      ...event,
      end: roundToNearestMinutes(addMinutes(mouseDate, offsetFromEnd.current), { nearestTo: roundMinutes })
    })
